...
public class HexComponent extends JComponent {
	...
	protected void paintComponent(Graphics g) {
		...
		if (c == HexColors.WALL) {
			...
			displayed = true;
		} else if (c == HexColors.END_PROCESSED || c == HexColors.START_PROCESSED || c == HexColors.START_POPPED) {
			Image img;
			gp = new GradientPaint(hexX[5], hexY[5], palette.initialGradient(c), hexX[1], hexY[1],
					palette.gradientColor(c), true);
			g2d.setPaint(gp);
			g2d.fillPolygon(hexagon);
			if (c == HexColors.END_PROCESSED) {
			...
			displayed = true;
		} else if (c == HexColors.LAVA || c == HexColors.LAVA_PUSHED || c == HexColors.LAVA_POPPED) {
			try {
				BufferedImage texture;
				if (c == palette.LAVA)
					texture = ImageIO.read(new File("lava.jpg"));
				else if (c == palette.LAVA_PUSHED)
		...
		} else if (c == HexColors.CACTUS || c == HexColors.CACTUS_PUSHED || c == HexColors.CACTUS_POPPED) {
			try {
				BufferedImage texture;
				if (c == HexColors.CACTUS)
					texture = ImageIO.read(new File("cactus.jpg"));
				else if (c == HexColors.CACTUS_PUSHED)
